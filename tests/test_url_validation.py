...
class TestURLSecurityValidation:
    ...
    def test_xss_prevention(self):
        ...
        xss_attempts = [
            ...
            ("https://evil.com/</script><script>alert('xss')</script>", "Script tag injection"),
            ("https://evil.com/<iframe src='javascript:alert(1)'></iframe>", "Iframe injection"),
            ("https://evil.com/<object data='javascript:alert(1)'></object>", "Object injection"),
            ("https://evil.com/<embed src='javascript:alert(1)'></embed>", "Embed injection"),
            ("https://evil.com/<link rel='stylesheet' href='javascript:alert(1)'>", "Link injection"),
            ("https://evil.com/<meta http-equiv='refresh' content='0;javascript:alert(1)'>", "Meta injection"),
            ...
            ("https://evil.com/page\rinjection", "Carriage return injection")  # 实际的回车符
        ]
        
        for url, description in xss_attempts:
            is_valid, error = validate_url_security(url)
            assert not is_valid, f"XSS attempt should be rejected ({description}): {url}"
    ...
    def test_ssrf_prevention(self):
        ...
        for url, description in ssrf_attempts:
            is_valid, error = validate_url_security(url)
            assert not is_valid, f"SSRF attempt should be rejected ({description}): {url}"
    ...
    def test_edge_cases(self):
        ...
        # 测试最大允许长度
        max_length_url = "https://example.com/" + "a" * (2048 - len("https://example.com/"))
...
class TestComprehensiveURLSecurity:
    ...
    def test_protocol_bypass_attempts(self):
        ...
        for attempt in bypass_attempts:
            ...
            assert not is_valid, f"Protocol bypass attempt should be rejected: {attempt}"
    ...
    def test_private_ip_detection(self):
        ...
        for ip_url in private_ips:
            ...
            assert ("private" in error.lower() or "local" in error.lower() or 
                   "loopback" in error.lower() or "not allowed" in error.lower())
    ...
    def test_domain_validation_integration(self):
        ...
        for url in valid_format_urls:
            ...
            assert is_valid, f"Properly formatted URL should pass security validation: {url}"
