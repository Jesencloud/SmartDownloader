...
def test_get_video_info_rejects_non_whitelisted_domain(client):
    ...
    json_response = response.json()
    assert "detail" in json_response
    assert "Downloads from 'www.some-random-site.com' are not permitted" in json_response["detail"]
    # 确保不再显示白名单列表
    assert "Only downloads from the following sites are permitted" not in json_response["detail"]
