...
def try_auto_extract_cookies(
    ...
    try:
        ...
        console.print(
            f"🍪 正在为 {first_url} 从{browser_type}浏览器自动提取cookies...",
            style="cyan",
        )
...
def handle_browser_mode_cookies(
    inputs: List[str], browser_type: str, cookies_config, force_refresh: bool
) -> Optional[str]:
    """处理浏览器模式cookies。
...
def handle_auto_mode_cookies(
    ...
    if cookies_config.auto_extract_enabled:
        console.print(
            "🔍 未找到手动cookies文件和缓存，尝试自动获取浏览器cookies...",
            style="yellow",
        )
        if inputs:
            first_url = inputs[0]
...
async def process_download_tasks(
    ...
    sub_processor: Optional[SubtitleProcessor],
    inputs: List[str],
